...
const features = [
  {
    icon: Truck,
    title: 'Free Shipping',
    description: 'On all orders above ₦50,000. Delivered to your door nationwide.',
  },
  {
    icon: ShieldCheck,
    title: 'Secure Payments',
    description: 'Every transaction is encrypted and protected by Paystack.',
  },
  {
    icon: RefreshCw,
    title: 'Easy Returns',
    description: '30-day hassle-free returns on all eligible items.',
  },
  {
    icon: Headphones,
    title: '24/7 Support',
    description: 'Our customer success team is always ready to help.',
  },
]
...
const categories = [
  { name: 'Electronics', icon: '💻', href: '/products?category=electronics', color: 'bg-blue-50 dark:bg-blue-900/20' },
  { name: 'Fashion', icon: '👗', href: '/products?category=fashion', color: 'bg-pink-50 dark:bg-pink-900/20' },
  { name: 'Home & Living', icon: '🏠', href: '/products?category=home', color: 'bg-amber-50 dark:bg-amber-900/20' },
  { name: 'Sports', icon: '⚽', href: '/products?category=sports', color: 'bg-green-50 dark:bg-green-900/20' },
  { name: 'Beauty', icon: '✨', href: '/products?category=beauty', color: 'bg-purple-50 dark:bg-purple-900/20' },
  { name: 'Books', icon: '📚', href: '/products?category=books', color: 'bg-orange-50 dark:bg-orange-900/20' },
]
  ...
  return (
    <>
            ...
            <div className="flex items-center gap-6 mt-10">
              <div className="flex -space-x-2">
                {[...Array(4)].map((_, i) => (
                  <div
                    key={i}
                    className="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-accent border-2 border-slate-800 flex items-center justify-center text-xs font-bold text-white"
                  >
      ...
      <section className="section">
        <div className="container-base">
          <div className="flex items-end justify-between mb-8">
            ...
            <Link
              to="/categories"
              className="text-sm font-medium text-primary hover:text-primary-hover transition-colors flex items-center gap-1 hidden sm:flex"
            >
        ...
        <div className="container-base">
          <div className="flex items-end justify-between mb-8">
            ...
            <Link
              ...
              className="text-sm font-medium text-primary hover:text-primary-hover transition-colors items-center gap-1 hidden sm:flex"
            >
              View all <ArrowRight className="w-3.5 h-3.5" />
            </Link>
          </div>

          {/* Product grid placeholder — replaced in Phase 2 */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="card group cursor-pointer hover:shadow-card-hover transition-shadow duration-200">
                <div className="aspect-square rounded-lg bg-background mb-4 flex items-center justify-center">
                  <Package className="w-12 h-12 text-text-muted" />
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-text-muted">Category</p>
                  <p className="font-medium text-text-primary">Product Name</p>
                  <p className="text-sm font-bold text-primary">₦12,000</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="section bg-surface border-y border-border">
        <div className="container-base">
          <p className="text-center text-sm text-text-muted mb-8 font-medium uppercase tracking-widest">
            Trusted brands
          </p>
          <div className="flex flex-wrap items-center justify-center gap-8 md:gap-12">
            {['Nike', 'Apple', 'Samsung', 'Sony', 'Adidas', 'LG'].map(brand => (
              <Link
                key={brand}
                to={`/brands/${brand.toLowerCase()}`}
                className="text-xl font-bold text-text-muted hover:text-text-secondary transition-colors"
              >
                {brand}
              </Link>
            ))}
          ...
          <div className="rounded-2xl bg-gradient-to-r from-primary to-accent p-8 md:p-12 text-white text-center md:text-left md:flex md:items-center md:justify-between gap-8">
            <div>
              ...
              <p className="text-blue-100 mb-0">
                Sign up now and use code <strong>WELCOME10</strong> at checkout.
              </p>
            </div>
